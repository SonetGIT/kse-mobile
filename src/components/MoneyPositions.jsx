...
export default function MoneyPositions(props) {
  const cls = useStyles();
  ...
  async function getActiveBids(){
    let docList = await fetch(kseRESTApi + "/api/Accounts/ViewCurrencies", 
      {
        ...
        <Grid>        
          ...
          >
            <IconButton style={{padding:1, fontSize:19}} onClick={()=>LeftClick(currIndex)}>
              ...
              <tr style={{fontSize:10, clolor:'#757575', maxWidth:50, borderColor:'white', textAlign:'center'}}>{currIndex +1} / {docList.length} </tr>
